...
// فقط محصولاتِ status-only (بدونِ manage_stock) که «ناموجود» نیستند تغییر می‌کنند.
// قاعدهٔ مالک: محصولی که موجودیِ عددی دارد (manage_stock=yes) هرگز دست نمی‌خورد.
// خروجی: OK<TAB>id یا SKIP-managed یا ERR<TAB>id<TAB>msg. برای برگرداندن از بکاپِ TSV + restore_stock.php.
...
$ok = 0; $skip = 0; $managed = 0; $err = 0;
foreach ($ids as $id) {
  $id = (int)$id;
  try {
    $p = wc_get_product($id);
    if (!$p) { echo "ERR\t$id\tnotfound\n"; $err++; continue; }
    if ($p->get_manage_stock()) { $managed++; continue; }  // موجودیِ عددی → دست نزن (قاعدهٔ مالک)
...
fwrite(STDERR, "done ok=$ok skip=$skip managed_untouched=$managed err=$err\n");
