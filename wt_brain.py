...
async def followup_questions(name: str, done: str, opent: str, report: str, store: str = "", directives: str = "") -> str:
    ...
    system = (
        ...
        "رشد و ارتقای فروش است. می‌خواهی هیچ ابهامی درباره‌ی کارِ امروز، «کارِ مانده» و مسیرِ رشد باقی نماند. با لحنِ گرم، "
        "مهربان و دلگرم‌کننده (نه بازجویانه — انگار کنارِ کارمند و طرفدارِ اویی)، ۲ تا ۴ سؤالِ کوتاه و مشخص بپرس که: "
        "هر سؤال را نرم و محترمانه و با یک قدردانیِ کوچک شروع کن (مثلاً «دمت گرم، فقط یه لحظه بگو…»، «ممنون می‌شم روشن کنی…»)، "
        "از لحنِ خشک یا صرفاً «چرا؟» پرهیز کن، ولی در همان جملهٔ مهربان کاملاً دقیق و عددی و مشخص بمان (بی‌رودربایستی و بدونِ گم‌شدنِ جزئیات). "
...
async def evaluate(name: str, done: str, opent: str, report: str, qa: str, store: str = "", directives: str = "") -> dict:
    ...
    system = (
        ...
        "لحن: گرم، مهربان و دلگرم‌کننده؛ مثلِ مدیری که آدم را می‌سازد — عادلانه و بدونِ تحقیر، با تشویقِ صمیمانه و یک نکته‌ی "
        "انگیزشیِ فروش‌محور. summary را با قدردانی شروع کن و متنِ هر تسک (label) را — که کارمند می‌بیند — دوستانه و روحیه‌بخش بنویس.\n"
...
async def route_issues(issues: list, staff: list) -> list:
    ...
    system = (
        ...
        "برای هر مشکل: key را عیناً تکرار کن، یک متنِ تسکِ کوتاه/عملی/فعل‌محور و با لحنِ دوستانه و دلگرم‌کننده بساز (کارمند "
        "آن را می‌بیند)، و فقط اگر واقعاً در حوزه‌ی شرحِ وظایفِ یک نفر است او را assignee بگذار؛ وگرنه assignee را خالی بگذار (حدس نزن). "
...
async def ig_content_plan(a: dict, inventory: dict | None = None, rivals: str = "", covered: str = "",
    ...
    system = (
        ...
        "۱) تقویم را فقط برای «روزهای خواسته‌شده» بده (نه بیشتر، نه کمتر). برای هر روز این‌ها را کامل و دقیق بده: "
        "نوعِ محتوا (پست/ریل/کاروسل) در type با تکیه بر فرمت‌های ترندِ روزِ اینستاگرام (ترنزیشن، قبل/بعد، POV، "
        "آنباکسینگ، آموزشِ سریع)، بهترین ساعتِ انتشار در time، یک «مدلِ مشخصِ موجود» از فهرستِ موجودی (نام/رفرنس) در "
        "brand، یک قلابِ کپشنِ فروش‌محور در hook، ۵ تا ۸ هشتگِ مرتبط در hashtags، در audio یک پیشنهادِ موزیک/صدا برای "
        "ریل (حس‌وحال + این تأکید که «از صداهای ترندِ همان‌روز از تبِ Reels» استفاده شود چون ریچ را بالا می‌برد)، و در "
        "stories «حداقل ۱۰ ایدهٔ استوریِ کوتاهِ آن روز» به‌صورتِ فهرستِ شماره‌دار که هرکدام یک مدلِ مشخصِ موجود "
        "(نام/رفرنس) را نشان می‌دهد و نوعش را بگوید (نظرسنجی/سؤال/بک‌استیج/مقایسه/قیمت‌محور/آموزشِ اصالت). هیچ روزی را ناقص نگذار.\n"
        "۲) planِ پوششِ برند/رفرنس: کدام برند/مدل‌های موجود کم‌دیده شده‌اند و باید بیشتر بیایند تا همهٔ رفرنس‌های "
        "موجود در طولِ زمان پوشش داده شوند. فقط مدل‌های واقعاً موجود؛ برندِ من‌درآوردی نساز.\n"
        "۳) اگر دادهٔ «رقبا» داده شد: نقاطِ قوتشان را متمایزتر و بهتر اجرا کن و شکافِ محتوایی/برندی را پر کن تا از "
        "آن‌ها جلو بزنیم (کپی نکن؛ برتری بساز).\n"
        "لحن: حرفه‌ای، دقیق و دلگرم‌کننده. فقط و فقط یک JSON با این کلیدها، بدونِ متنِ اضافه:\n"
        "{\"summary\":\"...\",\"calendar\":[{\"day\":\"...\",\"type\":\"...\",\"brand\":\"...مدل/رفرنسِ موجود...\","
