...
async def ig_content_plan(a: dict, inventory: dict | None = None, rivals: str = "", covered: str = "") -> dict:
    ...
    system = (
        ...
        "۱) تقویمِ ۷روزه (شنبه تا جمعه): برای هر روز این‌ها را کامل بده — نوعِ محتوا (پست/ریل/کاروسل/استوری) در type، "
        "بهترین ساعتِ انتشار در time، یک «مدلِ مشخصِ موجود» از فهرستِ موجودیِ فروشگاه (نام/رفرنس) در brand، "
        "یک قلابِ کپشنِ کوتاهِ فروش‌محور در hook، ۵ تا ۸ هشتگِ مرتبطِ فارسی/انگلیسی در hashtags، و یک ایدهٔ استوری در "
        "story. هیچ روزی را ناقص/خالی نگذار. از بهترین نوع/ساعت/روزِ واقعی استفاده کن.\n"
        ...
        "{\"summary\":\"...\",\"calendar\":[{\"day\":\"...\",\"type\":\"...\",\"brand\":\"...\",\"time\":\"...\","
        "\"hook\":\"...\",\"hashtags\":\"...\",\"story\":\"...\"}],\"brand_plan\":[\"...\"],\"tasks\":[\"...\"]}"
    )
    user = (
    ...
    try:
        raw = (await _chat(system, user, 9000, effort="high")).strip()  # موتورِ ۵.۵ با استدلالِ کامل
        if raw.startswith("```"):
            raw = raw.strip("`")
            if raw[:4].lower() == "json":
                raw = raw[4:]
